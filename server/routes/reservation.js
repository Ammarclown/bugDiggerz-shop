...
reserveRoutes.route("/reserved/:matchNO/:cno/:capacity").patch(function (req, response) {
...
reserveRoutes.route("/pending/:matchNO/:cno/:capacity").patch(async function (req, response) {
...
if(Number(req.params.cno)==1 && cat1< (pend1+inc)){
  response.send("TICKET SOLDOUT IN CATEGORY 1")
}
...
if(Number(req.params.cno)==2 && cat2< (pend2+inc)){
  response.send("TICKET SOLDOUT IN CATEGORY 2")
}
...
if(Number(req.params.cno)==3 && cat3< (pend3+inc)){
  response.send("TICKET SOLDOUT IN CATEGORY 3")
}
//console.log(Number(myquery._id.availability.category1.count))
 
 db_connect
   .collection("updatedMasterlist")
   .updateOne(myquery, newvalues, function (err, res) {
     if (err) throw err;
     console.log("1 document updated for reservation");
     response.json(res);
...
reserveRoutes.route("/cancel/:matchNO/:cno/:capacity").patch(function (req, response) {
  let decrement=Number(req.params.capacity)*-1
 let db_connect = dbo.getDb("worldcup22");
 let myquery = { "matchNumber": Number(req.params.matchNO),
"availability.category1.price":75 };
